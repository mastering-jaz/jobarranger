                      ...
                      const char *script)
{
    ...
    zabbix_log(LOG_LEVEL_DEBUG,
               "In %s() filepath: %s", __function_name, filepath);
    ...
    fp = fopen(filename, "w");
    if (fp == NULL) {
        zabbix_log(LOG_LEVEL_ERR, "Can not open script file: %s (%s)",
    ...
    fclose(fp);
    if (err < 0) {
        zabbix_log(LOG_LEVEL_ERR, "Can not write to script file: %s (%s)",
                   filename, strerror(errno));
        return FAIL;
    }

    if (chmod(filename, 0755) < 0) {
        zabbix_log(LOG_LEVEL_ERR, "Can not chmod script file: %s (%s)",
                   filename, strerror(errno));
        return FAIL;
    }
    //create result files
    i = 0;
    while (jobext[i] != NULL) {
        zbx_snprintf(filename, sizeof(filename), "%s.%s", filepath,
                     jobext[i]);
        if (strcmp(jobext[i], "stdout") == 0
            || strcmp(jobext[i], "stderr") == 0)
            ret = ja_file_create(filename, JA_MAX_DATA_LEN);
        else
...
int ja_jobfile_chkend(const char *filepath, JA_PID pid)
{
    ...
    zabbix_log(LOG_LEVEL_DEBUG,
               "In %s() filepath: %s, pid: %d", __function_name, filepath,
               pid);
    ...
    if (ret_fsize < 0 || start_fsize < 0 || end_fsize < 0) {
        zabbix_log(LOG_LEVEL_ERR,
                   "In %s() more than one of the result files are not existed. ret_fsize: %d, start_fsize: %d, end_fsize: %d ",
    ...
    zabbix_log(LOG_LEVEL_ERR,
               "In %s() process %d isn't existed", __function_name, pid);
    return -1;
}
...
int ja_jobfile_load(const char *filepath, ja_job_object * job)
{
    ...
    zbx_snprintf(filename, JA_MAX_STRING_LEN, "%s.start", filepath);
    if (ja_file_load(filename, sizeof(t), &(t))
        == FAIL)
        return FAIL;
    job->start_time = t;
